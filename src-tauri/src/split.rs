...
//!
//! **Cópia literal do `split.rs` do LocalZip v0.5.0** (mesmo autor, mesma
//! suíte, mesma licença MIT). Não foi reescrito de propósito: o comportamento
//! tem que ser IDÊNTICO ao do LocalZip, senão o mesmo `.zip.001` abriria de um
//! jeito no gerenciador de arquivos e de outro no compactador. Correção feita
//! num dos dois se aplica copiando o arquivo inteiro.
//!
...
impl SplitReader {
    ...
    ///
    /// Sem uso no LocalFiles (o LocalZip usa pra calcular a razão de expansão
    /// do alarme de zip bomb). Fica porque este arquivo é cópia LITERAL do
    /// LocalZip de propósito — podar aqui obrigaria a comparar os dois na mão
    /// na próxima correção.
    #[allow(dead_code)]
